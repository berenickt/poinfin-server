...
export class PaymentsService {
  ...
  findOneByImpUid({ impUid }: IPaymentServiceFindOneByImpUid): Promise<Payment> {
    return this.paymentRepository.findOne({ where: { impUid } })
  }

  async checkDuplication({ impUid }: IPaymentServiceCheckDuplication): Promise<void> {
    const result = await this.findOneByImpUid({ impUid })
    if (result) throw new ConflictException('이미 등록된 결제 아이디입니다.')
  }

  async create({ createPaymentInput, user }: IPaymentsServiceCreate): Promise<Payment> {
  ...
  async createFreeSeries({ seriesList, user }: IPaymentsServiceCreateFreeSeries): Promise<Payment> {
    // 결제 내역 저장
