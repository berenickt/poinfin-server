...
import { SeriesCategoriesService } from '../series-categories/series-categories.service'
...
export class SeriesService {
  ...
  async findOne({ seriesId }: ISeriesServiceFindOne): Promise<IFetchSeriesReturn> {
  ...
  findAllByCart({ seriesList }: ISeriesServicefindAllByCart): Promise<Series[]> {
    return this.seriesRepository.find({
  ...
  findByCategory({ categoryId }: ISeriesServiceFindByCategory): Promise<Series[]> {
    return this.seriesRepository.find({
  ...
  async create({ createSeriesInput, user }: ISeriesServiceCreate): Promise<Series> {
    const { categoryId, posts, ...rest } = createSeriesInput
    const category = await this.seriesCategoriesService.findOne({ categoryId })

    if (!category) throw new UnprocessableEntityException("Category dosen't exist")

    if (posts.length == 0) throw new UnprocessableEntityException("Post dosen't exist")
  ...
  async update({ updateSeriesInput, seriesId }: ISeriesServiceUpdate): Promise<Series> {
